...
class Benchmark:
    ...
    def set_axes_data(self, axes, data=None):
        self._axes = axes
        self.grid = cart(self._axes)
    ...
    def eval(self):
        self.eval_model = self.model(self._data).detach().numpy()
        self.eval_model = self.eval_model.T[0].reshape([len(_) for _ in self._axes])

        if self._gt is not None:
            self.eval_gt = self._gt(
                *[self.grid[:, i] for i in range(len(self._axes))]).reshape([len(_) for _ in self._axes])

            if torch.is_tensor(self.eval_gt):
                self.eval_gt = self.eval_gt.detach().numpy()
